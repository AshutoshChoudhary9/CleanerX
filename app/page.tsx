    ...
    <>
      ...
      <nav>
        <div className="nav-inner">
          ...
          <div className="nav-actions">
            ...
            <button className="nav-btn" onClick={() => showToast(`${wishlist.size} items wishlisted!`, 'success')}>
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="subnav">
        <div className="subnav-inner">
          <div className={`snav-item ${activeCategory === 'all' ? 'active' : ''}`} onClick={() => handleFilter('all')}>All Products</div>
          <div className={`snav-item ${activeCategory === 'floor' ? 'active' : ''}`} onClick={() => handleFilter('floor')}>🧹 Floor Cleaners</div>
          <div className={`snav-item ${activeCategory === 'toilet' ? 'active' : ''}`} onClick={() => handleFilter('toilet')}>🚽 Toilet Cleaners</div>
          <div className={`snav-item ${activeCategory === 'glass' ? 'active' : ''}`} onClick={() => handleFilter('glass')}>🪟 Glass Cleaners</div>
          <div className={`snav-item ${activeCategory === 'combo' ? 'active' : ''}`} onClick={() => handleFilter('combo')}>📦 Combo Packs</div>
          <div className="snav-item" onClick={() => showToast('Festive deals loaded!', 'success')}>🎉 Festive Deals</div>
          <div className="snav-item" onClick={() => showToast('Bulk order discount applied!', 'success')}>🏭 Bulk Orders</div>
          <div className="snav-item" onClick={() => showToast('Subscription coming soon!', 'success')}>🔁 Subscribe & Save</div>
        </div>
      </div>
      ...
      <div className="hero">
        <div className="hero-left">
          ...
          <div className="hero-ctas">
            ...
            <button className="btn-outline" onClick={() => showToast('Exploring our range!', 'success')}>View All Products</button>
          </div>
        </div>
        <div className="hero-right">
          {[
            ...
            <div key={hc.cat} className="hero-product-card" onClick={() => handleFilter(hc.cat)}>
      ...
      <div className="main">
        ...
        <div className="cat-grid">
          {[
            { key: 'floor', name: 'Floor Cleaners', icon: '🧹', desc: 'Powerful disinfection for marble, tile, granite & wooden floors. Removes stains and leaves a fresh fragrance.', count: 12 },
            { key: 'toilet', name: 'Toilet Cleaners', icon: '🚽', desc: 'Thick gel formula that clings to surfaces, removes stains & limescale. 100% germ-free guarantee.', count: 8 },
            { key: 'glass', name: 'Glass Cleaners', icon: '🪟', desc: 'Streak-free, crystal-clear finish on windows, mirrors & glass surfaces. Dries instantly.', count: 10 },
          ].map(cat => (
            <div key={cat.key} className={`cat-card ${cat.key}`} onClick={() => handleFilter(cat.key)}>
              <div className="cat-icon">{cat.icon}</div>
              <div>
                <div className="cat-name">{cat.name}</div>
                <div className="cat-desc">{cat.desc}</div>
                <div className="cat-count">{cat.count} Products Available</div>
              </div>
        ...
        <div className="deals-section">
          <div className="deals-header">
            <div className="deals-title">🔥 Deal of the Day</div>
        ...
        <div ref={productsRef} id="products">
          <div className="section-header">
            <div className="section-title">
              {activeCategory === 'all' ? 'All' : activeCategory.charAt(0).toUpperCase() + activeCategory.slice(1)} <span>Products</span>
            </div>
            <div className="see-all" onClick={() => handleFilter('all')}>View All ›</div>
          </div>
                 ...
                 return (
                  <div key={p.id} className="prod-card" onClick={() => { setSelectedProduct(product); setModalState('product'); }}>
                    <div className="prod-img">
                      ...
                      <div className={`prod-wishlist ${inWish ? 'active' : ''}`} onClick={(e) => { e.stopPropagation(); toggleWishlist(p.id); }}>
                        {inWish ? '❤️' : '🤍'}
                      </div>
                    </div>
        ...
        {/* ── ADD PRODUCT SECTION ── */}
        <div className="add-product-section" style={{ marginTop: 56 }}>
          <div className="section-header">
            <div className="section-title">Add Your <span>Product Listings</span></div>
          </div>
          <div className="add-grid">
            {[
              ...
              <div key={box.cat} className="add-box">
                 <h3>{box.label}</h3>
                 <div className="field"><label>Product Name</label><input type="text" placeholder={box.placeholder} value={addFormVisible === box.cat ? addForm.name : ''} onChange={e => {setAddFormVisible(box.cat); setAddForm(f => ({ ...f, name: e.target.value, category: box.cat }))}} /></div>
                 <div className="field"><label>Volume</label><input type="text" placeholder="e.g. 1 Litre" value={addFormVisible === box.cat ? addForm.vol : ''} onChange={e => {setAddFormVisible(box.cat); setAddForm(f => ({ ...f, vol: e.target.value, category: box.cat }))}} /></div>
                 <div className="field"><label>MRP (₹)</label><input type="number" placeholder="e.g. 199" value={addFormVisible === box.cat ? addForm.mrp : ''} onChange={e => {setAddFormVisible(box.cat); setAddForm(f => ({ ...f, mrp: e.target.value, category: box.cat }))}} /></div>
                 <div className="field"><label>Selling Price (₹)</label><input type="number" placeholder="e.g. 149" value={addFormVisible === box.cat ? addForm.price : ''} onChange={e => {setAddFormVisible(box.cat); setAddForm(f => ({ ...f, price: e.target.value, category: box.cat }))}} /></div>
                 <div className="field"><label>Fragrance / Type</label><input type="text" placeholder={box.fragrancePlaceholder} value={addFormVisible === box.cat ? addForm.fragrance : ''} onChange={e => {setAddFormVisible(box.cat); setAddForm(f => ({ ...f, fragrance: e.target.value, category: box.cat }))}} /></div>
                 <div className="field"><label>Description</label><textarea placeholder="Product highlights, features..." value={addFormVisible === box.cat ? addForm.description : ''} onChange={e => {setAddFormVisible(box.cat); setAddForm(f => ({ ...f, description: e.target.value, category: box.cat }))}} /></div>
                 <button className="add-btn" onClick={() => handleAddProduct(box.cat)}>+ Add to Store</button>
              </div>
            ))}
          </div>
        </div>
                ...
                <>
                  ...
                  {checkoutStep === 1 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div className="form-group"><label>Mobile Number</label><input type="tel" placeholder="10-digit mobile number" /></div>
                      <div className="form-group"><label>Address Line 1</label><input type="text" placeholder="House/Flat/Block No." /></div>
                      <div className="form-row">
                        <div className="form-group"><label>City</label><input type="text" placeholder="City" /></div>
                        <div className="form-group"><label>PIN Code</label><input type="text" maxLength={6} placeholder="6-digit PIN" /></div>
                      </div>
                      <div className="form-group"><label>State</label><select><option>Uttar Pradesh</option><option>Maharashtra</option><option>Delhi</option></select></div>
                    </div>
                  )}
